...
const CapitalsQuizQuestion = ({currentCountry, answers, receiveAnswer }) => {
    ...
    const quizAnswers = answers.map((answer) => {
        return (
            <div className="answers" key={answer.abbreviation} onClick={() => { receiveAnswer(answer.abbreviation) }}>{answer.capital}</div>
        )
    })

    return (
        <div>
        <img className="capitals_logo" height="250" src={`${process.env.PUBLIC_URL}/quiz.png`}  />
